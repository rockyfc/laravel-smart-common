...
class Parser
{
    ...
    public function parseInline($text, $whiteList = '', $clearHolders = true, $enableAutoLink = true)
    {
        ...
        return $this->call('afterParseInline', $text);
    }
    ...
    public function cleanUrl($url)
    {
        ...
        if (preg_match('/^\\s*([\\p{L}_a-z0-9-:\\.\\*\\/%#!@\\?\\+=~\\|\\,&]+)/iu', $url, $matches)) {
            return $matches[1];
        }
    ...
    private function parseBlockList($block, $key, $line, &$state)
    {
        if ($this->isBlock('list') && !preg_match('/^\\s*\\[((?:[^\\]]|\\]|\\[)+?)\\]:\\s*(.+)$/', $line)) {
            ...
            if (preg_match('/^(\\s*)$/', $line) && $state['empty'] == 0) {
                ++$state['empty'];
                $this->setBlock($key);
    ...
    private function parseBlockAhtml($block, $key, $line, &$state)
    {
        if ($this->_html) {
            ...
            } elseif ((bool)$state['html'] && strpos($line, "</{$state['html']}>") !== false) {
                $this->setBlock($key)->endBlock();
                $state['html'] = false;
    ...
    private function parseBlockPre($block, $key, $line, &$state)
    {
        ...
        if ($this->isBlock('pre') && preg_match('/^\\s*$/', $line)) {
            $this->setBlock($key);
    ...
    private function parseBlockHtml($block, $key, $line, &$state)
    {
        ...
        if (preg_match("/<\\/({$state['special']})>\\s*$/i", $line, $matches)) {
            $tag = strtolower($matches[1]);
